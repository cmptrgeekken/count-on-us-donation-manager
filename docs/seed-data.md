...
The former catalog/financial CSV seed importer was removed because it maintained
an independent cost engine that could produce snapshots inconsistent with normal
order processing. Use the deterministic seed commands above for development.
Use the admin **Imports & rebuild** workflow or `npm run backfill:financial` for
historical financial data so snapshots pass through the production cost engine.
