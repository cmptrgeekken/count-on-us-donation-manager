...
export function buildReportingPeriodCsv(summary: NonNullable<ReportingSummaryResult["summary"]>) {
  ...
  const rows: CsvRow[] = [
    ...
    },
    {
      ...
      section: "track1",
      recordType: "metric",
      metric: "externalSettlementFees",
      value: summary.track1.externalSettlementFees ?? "0",
    },
    {
      ...basePeriod,
      section: "track1",
      recordType: "metric",
      metric: "artistPayoutTotal",
      value: summary.track1.artistPayoutTotal ?? "0",
    },
    {
      ...basePeriod,
      section: "track1",
    ...
    ...(summary.artistAllocations ?? []).map<CsvRow>((allocation) => ({
      ...basePeriod,
      section: "artistAllocations",
      recordType: "artistAllocation",
      artistId: allocation.artistId,
      artistName: allocation.artistName,
      artistCreditName: allocation.creditName,
      allocatedAmount: allocation.allocated,
      disbursedAmount: allocation.paid,
      remainingAmount: new Prisma.Decimal(allocation.allocated)
        .sub(new Prisma.Decimal(allocation.paid))
        .toDecimalPlaces(2, Prisma.Decimal.ROUND_FLOOR)
        .toString(),
    })),
    ...(summary.artistPayables ?? []).flatMap<CsvRow>((payable) => [
      {
        ...basePeriod,
        section: "artistPayables",
        recordType: "artistPayable",
        artistId: payable.artistId,
        artistName: payable.artistName,
        artistCreditName: payable.creditName,
        currentOutstanding: payable.currentOutstanding,
        priorOutstanding: payable.priorOutstanding,
        totalOutstanding: payable.totalOutstanding,
        notes: payable.overdue ? "Overdue" : "",
      ...
      ...payable.periods.map((period) => ({
        ...basePeriod,
        section: "artistPayables",
        recordType: "artistPayablePeriod",
        artistId: payable.artistId,
        artistName: payable.artistName,
        artistCreditName: payable.creditName,
        periodId: period.periodId,
    ...
    ...(summary.externalSettlements ?? []).map<CsvRow>((settlement) => ({
      ...basePeriod,
      section: "externalSettlements",
      recordType: "externalSettlement",
      description: `${settlement.orderNumber ?? settlement.shopifyOrderId} (${settlement.source}, ${settlement.status})`,
      paidAt: formatDate(settlement.paidAt),
      referenceId: settlement.referenceId ?? "",
      value: settlement.feeAmount,
      allocatedAmount: settlement.amountReceived ?? "",
      notes: settlement.notes ?? settlement.detectedReason ?? "",
    })),
    ...
    ...(summary.artistPayments ?? []).map<CsvRow>((payment) => ({
      ...basePeriod,
      section: "artistPayments",
      recordType: "artistPayment",
      artistId: payment.artistId,
      artistName: payment.artistName,
      artistCreditName: payment.creditName,
      paidAt: formatDate(payment.paidAt),
      paymentMethod: payment.paymentMethod,
      referenceId: payment.referenceId ?? "",
      value: payment.amount,
      notes: payment.notes ?? "",
      applicationPeriods: payment.applications
        .map((application) => `${formatDate(application.periodStartDate)}..${formatDate(application.periodEndDate)}=${application.amount}`)
        .join(" | "),
    })),
    {
...
export function buildReportingPeriodPdf(summary: NonNullable<ReportingSummaryResult["summary"]>) {
  const lines = [
    ...
    "",
    "Artist payout allocations",
    ...(summary.artistAllocations ?? []).map((allocation) => `- ${allocation.creditName}: allocated ${allocation.allocated}, paid ${allocation.paid}`),
    "",
    "Outstanding artist payables",
    ...(summary.artistPayables ?? []).map((payable) => `- ${payable.creditName}: current ${payable.currentOutstanding}, prior ${payable.priorOutstanding}, total ${payable.totalOutstanding}`),
    "",
    "Disbursements",
    ...summary.disbursements.map((disbursement) => `- ${disbursement.causeName} paid ${formatDate(disbursement.paidAt)} total ${disbursement.amount}${disbursement.paymentMethod ? ` via ${disbursement.paymentMethod}` : ""}`),
    "",
    "Artist payments",
    ...(summary.artistPayments ?? []).map((payment) => `- ${payment.creditName} paid ${formatDate(payment.paidAt)} total ${payment.amount} via ${payment.paymentMethod}`),
    "",
    "Shopify charges",
    ...summary.charges.map((charge) => `- ${charge.description}: ${charge.amount}`),
    "",
    "External settlements",
    ...(summary.externalSettlements ?? []).map((settlement) => `- ${settlement.orderNumber ?? settlement.shopifyOrderId} ${settlement.source}/${settlement.status}: fee ${settlement.feeAmount}, received ${settlement.amountReceived ?? "not entered"}`),
    "",
    "Tax true-ups",
