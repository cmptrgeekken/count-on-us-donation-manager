...
import { buildPublicArtistsDirectory } from "../services/publicMerchandising.server";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const rateLimit = checkRateLimit({
    key: `public-artists:${shopifyDomain}:${getClientIpAddress(request)}`,
    limit: 60,
    windowMs: 60_000,
  });

  if (!rateLimit.allowed) {
    throw new Response("Too many artist directory requests. Please try again shortly.", {
      status: 429,
      headers: rateLimit.headers,
    });
  }

  const payload = await buildPublicArtistsDirectory(shopifyDomain);
